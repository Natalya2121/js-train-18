...
function checkData(data) {
  ...
  try {
    if (Object.keys(data).length === 0)
      throw new Error("Об'єкт пустий", { cause: data });
    return data;
  } catch (err) {
    return err.message;
  }
...
function getAge(age) {
  ...
  try {
    if (age < 0) {
      let err1 = new Error("Вік не може бути менше 0!");
      err1.name = "AgeError";
      throw err1;
    }
...
function decodeURIComponentWrapper(encodedString) {
  ...
  try {
    let uri = decodeURI(encodedString);
    return uri;
  } catch (err) {
  ...
  try {
    for (let idx in data) {
      if (typeof data[idx] !== "number")
        throw new TypeError(`Елемент з індексом ${idx} має бути числом!`);
    }
...
function evaluateExpression(expression) {
  ...
  try {
    ...
    return new EvalError(err.message, err);
  }
}
